...
public class EnPassant extends SinglePieceTake {
	...
	public Position pawnLocation(Board board) {
		if (board.pieceAt(oldPosition).isWhite()) {
			return new Position(newPosition.row(),newPosition.column()-1 );
		}
		else return new Position(newPosition.row(),newPosition.column()+1 );
	}
	...
	public boolean isValid(Board board) {
		...
		if (board.pieceAt(newPosition) instanceof Pawn) {
			return true;
		}
		return false;
	}
